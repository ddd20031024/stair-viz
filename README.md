...
| 项目 | 说明 |
|---|---|
| 物理层 | USB 转 TTL 串口（Windows: `COMx`, Linux: `/dev/ttyACM0`） |
| 波特率 | 921600, 8N1 |
| 读请求 | `0x55` \| `CMD(0x01)` \| `REG_ADDR(1B)` \| `LEN(1B)` \| `CRC8(1B)` |
| 读响应 | `DATA(4B×LEN)` \| `CRC8(1B)`（无帧头） |
| 写请求 | `0x55` \| `CMD(0x02)` \| `REG_ADDR(1B)` \| `LEN(1B)` \| `DATA(4B×LEN)` \| `CRC8(1B)` |
| 字节序 | Little-Endian |
| CRC | CRC-8，多项式 `0x07` |
| 寄存器位宽 | 32-bit / 寄存器 |
| 轮询模式 | 主机主动轮询，MCU 响应（非持续流式推送） |

完整寄存器列表与协议说明见 `../robot_win/QLRobot通信协议与寄存器说明.md`。

## 3. 可视化面板

| 面板 | 数据来源（寄存器地址） | 刷新率 | 内容 |
|---|---|---|---|
| 安全状态 | CTRL(0x00), ONLINE(0x52), TOF(0x11-0x14) | 100Hz | 底盘模式、连接状态、错误标志位、TOF 距离摘要 |
| TOF 测距 | TOF1-4(0x11-0x14) | 50Hz | 4 路 TOF 距离柱状图、信号强度、有效状态 |
| IMU 姿态 | IMU_PITCH/ROLL(0x1A-0x1B) + IMU 全量(0x15-0x22) | 50/20Hz | Pitch/Roll 人工地平线、底盘俯仰、温度 |
| FOC 电机 | MOTOR_L3~R2(0x23-0x2E) | 20Hz | 6 路电机在线/速度/扭矩 |
| 三角轮 | TRIWHEEL_CUR(0x2F-0x32) | 20Hz | 4 轮角度 + 占空比、底盘俯仰估计 |
...
stair_viz/
  viz_dashboard/
    __init__.py
    protocol.py              ← FlexWire 寄存器读写 + CRC8 + 寄存器字段定义
...
# 4. 真实串口模式（Windows）
python -m viz_dashboard.viz_server --port COM3
...
| 参数 | 默认值 | 说明 |
|---|---|---|
| `--port` | 无（模拟模式） | 串口设备路径，如 `COM3` (Windows) 或 `/dev/ttyACM0` (Linux) |
| `--baud` | `921600` | 虚拟串口波特率 |
...
- [MCU 通信协议与寄存器说明](../robot_win/QLRobot通信协议与寄存器说明.md)
- [协议变更对照表](协议变更对照表.md)
